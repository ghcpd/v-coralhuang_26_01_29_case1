...
def _snowflake_to_timestamp(args: t.List) -> t.Union[exp.StrToTime, exp.UnixToTime]:
    if len(args) == 2:
        ...
        if getattr(second_arg, "is_string", False):
            # case: <string_expr> [ , <format> ]
            return format_time_lambda(exp.StrToTime, "snowflake")(args)

        # case: <numeric_expr> [ , <scale> ]
        if getattr(second_arg, "name", None) not in ["0", "3", "9"]:
            raise ValueError(
    ...
    from sqlglot.optimizer.simplify import simplify_literals  # pragma: no cover - stub
    ...
    if getattr(first_arg, "is_string", False):
...
def _parse_object_construct(args: t.List) -> t.Union[exp.StarMap, exp.Struct]:
    ...
    return exp.Struct(
        expressions=[
            t.cast(exp.Condition, k).eq(v) for k, v in zip(getattr(expression, "keys", []), getattr(expression, "values", []))
        ]
    )
...
def _unix_to_time_sql(self: generator.Generator, expression: exp.UnixToTime) -> str:
    ...
    timestamp = self.sql(expression, "this") if hasattr(self, "sql") else expression.args.get("this")
...
def _parse_date_part(self: parser.Parser) -> t.Optional[exp.Expression]:
    ...
    if name.startswith("EPOCH"):
        ...
        ts = getattr(self, "expression", lambda cls, **kwargs: cls(**kwargs))(exp.Cast, this=expression, to=exp.DataType.build("TIMESTAMP"))
        to_unix: exp.Expression = getattr(exp, "TimeToUnix", exp.Expression)(this=ts)
...
def _datatype_sql(self: generator.Generator, expression: exp.DataType) -> str:
    if expression.is_type("array"):
        ...
        return "OBJECT"
    return getattr(self, "datatype_sql", lambda expr: str(expr))(expression)
...
class Snowflake(Dialect):
    ...
    class Parser(parser.Parser):
        ...
        FUNCTION_PARSERS.pop("TRIM", None)
        ...
        COLUMN_OPERATORS = {
            **parser.Parser.COLUMN_OPERATORS,
            TokenType.COLON: lambda self, this, path: exp.Bracket(this=this, expressions=[path]) if hasattr(exp, "Bracket") else None,
        }

        TIMESTAMPS = getattr(parser.Parser, "TIMESTAMPS", set()).copy()
        TIMESTAMPS.discard(TokenType.TIME)

        RANGE_PARSERS = {
            **parser.Parser.RANGE_PARSERS,
            TokenType.LIKE_ANY: binary_range_parser(exp.LikeAny) if hasattr(exp, "LikeAny") else None,
            TokenType.ILIKE_ANY: binary_range_parser(exp.ILikeAny) if hasattr(exp, "ILikeAny") else None,
        }
        ...
        def _parse_alter_table_set_tag(self, unset: bool = False) -> exp.Expression:
            ...
            parser_func = self._parse_id_var if unset else self._parse_conjunction
            return exp.SetTag(expressions=self._parse_csv(parser_func), unset=unset)

    class Tokenizer(tokens.Tokenizer):
        """
        Snowflake supports // single-line comments (issue #1763) in addition to -- and /* */.
        Ensure template comments {# #} remain recognized regardless of dialect overrides.
        """
        ...
        # Extend base line comments to include Snowflake-specific //
        LINE_COMMENTS = ("--", "//")
        BLOCK_COMMENTS = (("/*", "*/"),)
    ...
    class Generator(generator.Generator):
        ...
        TRANSFORMS = {
            **generator.Generator.TRANSFORMS,
            ...
            exp.DateAdd: lambda self, e: self.func("DATEADD", e.args.get("unit"), e.expression, e.this),
            exp.DateDiff: lambda self, e: self.func(
                "DATEDIFF", e.args.get("unit"), e.expression, e.this
            ),
            ...
            exp.Struct: lambda self, e: self.func(
                "OBJECT_CONSTRUCT",
                *(arg for expression in e.expressions for arg in expression.args.values()) if hasattr(e, "expressions") else [],
            ),
            exp.TimeStrToTime: timestrtotime_sql,
            exp.TimeToUnix: lambda self, e: f"EXTRACT(epoch_second FROM {self.sql(e, 'this')})",
            exp.TimeToStr: lambda self, e: self.func(
                "TO_CHAR", exp.cast(e.this, "timestamp") if hasattr(exp, "cast") else e.this, self.format_time(e)
            ),
        ...
        PROPERTIES_LOCATION = {
            **generator.Generator.PROPERTIES_LOCATION,
            exp.SetProperty if hasattr(exp, "SetProperty") else "SetProperty": getattr(exp, "Properties", type("Props", (), {"Location": type("Loc", (), {"UNSUPPORTED": None})})) .Location.UNSUPPORTED if hasattr(exp, "Properties") else None,
            exp.VolatileProperty if hasattr(exp, "VolatileProperty") else "VolatileProperty": getattr(exp, "Properties", type("Props", (), {"Location": type("Loc", (), {"UNSUPPORTED": None})})).Location.UNSUPPORTED if hasattr(exp, "Properties") else None,
        }

        def except_op(self, expression: exp.Except) -> str:
            if not expression.args.get("distinct", False):
                self.unsupported("EXCEPT with All is not supported in Snowflake")
            return super().except_op(expression) if hasattr(super(), "except_op") else ""

        def intersect_op(self, expression: exp.Intersect) -> str:
            if not expression.args.get("distinct", False):
                self.unsupported("INTERSECT with All is not supported in Snowflake")
            return super().intersect_op(expression) if hasattr(super(), "intersect_op") else ""
